...
def main() -> None:
    ...
    seed_path = Path("data/seeds") / args.dataset / "queried" / f"{args.target_model}_seed_{args.seed_size}_queried.parquet"
    mix_path = Path("data/mixup") / args.dataset / f"{args.target_model}_seed_{args.seed_size}_alpha_{args.alpha}.parquet"
    ...
    seed_train = seed_df[feature_cols + ["blackbox_label"]].copy()
    seed_train = seed_train.rename(columns={"blackbox_label": "target_label"})
    seed_train["data_source"] = "seed"

    mix_train = mix_df[feature_cols + ["y_a", "y_b", "lam"]].copy()
    mix_train["data_source"] = "mixup"
